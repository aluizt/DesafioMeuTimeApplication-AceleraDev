...
public class backupAnterior implements MeuTimeInterface {
	...
	public void incluirTime(Long id, String nome, LocalDate dataCriacao, String corUniformePrincipal, String corUniformeSecundario) {
		...
		if (listaTime.containsKey(id)) throw new IdentificadorUtilizadoException();	
	...
	public void incluirJogador(Long id, Long idTime, String nome, LocalDate dataNascimento, Integer nivelHabilidade, BigDecimal salario) {
		...
		if (listaJogador.containsKey(id)) throw new IdentificadorUtilizadoException();
		buscarTime(idTime).getJogadores().add(id);
	...
	public Long buscarCapitaoDoTime(Long idTime) {				
		...
		if (buscarTime(idTime).getIdCapitao() == null) 	throw new CapitaoNaoInformadoException();				
	...
	public List<Long> buscarJogadoresDoTime(Long idTime) {		
		return new ArrayList<>(buscarTime(idTime).getJogadores());		
	}

	@Desafio("buscarMelhorJogadorDoTime")
	public Long buscarMelhorJogadorDoTime(Long idTime) {
		
		Jogador jogadorMaisHabilidade = null;
		for (Long idJogador : buscarJogadoresDoTime(idTime)) {
			Jogador jogador = buscarJogador(idJogador);
			if (jogadorMaisHabilidade == null) {
				jogadorMaisHabilidade = jogador;
			} else {
				if (jogadorMaisHabilidade.getNivelHabilidade() < jogador.getNivelHabilidade()){
					jogadorMaisHabilidade = jogador;
				}
			}
		}

		return jogadorMaisHabilidade.getId();
	...
	public List<Long> buscarTopJogadores(Integer top) {
		if (listaJogador.size()==0) return new ArrayList<Long>();
		
		List<Jogador> jogadoresTop = new ArrayList<Jogador>(listaJogador.values());	
		
		Collections.sort(jogadoresTop, Comparator.comparing(Jogador::getNivelHabilidade).reversed()
	...
	public String buscarCorCamisaTimeDeFora(Long timeDaCasa, Long timeDeFora) {		
		Time timeLocal = buscarTime(timeDaCasa);
		Time timeVisitante = buscarTime(timeDeFora);		
		if(timeLocal.getCorUniformePrincipal()==timeVisitante.getCorUniformePrincipal()) return timeVisitante.getCorUniformeSecundario();
	...
	private Time buscarTime(Long idTime) {
		if (!listaTime.containsKey(idTime))  throw new TimeNaoEncontradoException();
		return listaTime.get(idTime);
	}

	private Jogador buscarJogador(Long idJogador) {
		if (!listaJogador.containsKey(idJogador)) 	throw new JogadorNaoEncontradoException();		
